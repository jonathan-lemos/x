...
verifyApplicativeHappyPathWithEqFn constructor name eq = basicAssertion $ do
    assert (((-) <$> constructor 9 <*> constructor 3) `eq` constructor 6) `withTitle` (name <> " (-) <$> " <> name <> " 9 <*> " <> name <> " 3 == " <> name <> " 6")
    assert (((<>) <$> constructor "foo" <*> constructor "bar") `eq` constructor "foobar") `withTitle` (name <> " (<>) <$> " <> name <> " \"foo\" <*> " <> name <> " \"bar\" == " <> name <> " \"foobar\"")
    assert (constructor 123 `eq` pure 123) `withTitle` (name <> " 123 == pure 123")
...
verifyApplicativeLawsWithEqFn constructor name eq = do
    ...
    basicAssertion $ do
        assert ((pure id <*> constructor 123) `eq` constructor 123) `withTitle` ("pure id <*> " <> name <> " 123 == " <> name <> " 123")
        assert ((pure id <*> constructor "abc") `eq` constructor "abc") `withTitle` ("pure id <*> " <> name <> " \"abc\" == " <> name <> " \"abc\"")

        assert ((pure show <*> pure 123) `eq` pure (show 123)) `withTitle` ("pure show <*> pure 123 == pure (show 123)")
        assert ((pure length <*> pure "abc") `eq` pure (length "abc")) `withTitle` ("pure length <*> pure \"abc\" == pure (show \"abc\")")

        assert ((pure show <*> pure 123) `eq` (pure ($ 123) <*> pure show)) `withTitle` ("pure show <*> pure 123 == pure ($ 123) <*> pure show")
        assert ((pure show <*> pure "abc") `eq` (pure ($ "abc") <*> pure show)) `withTitle` ("pure show <*> pure \"abc\" == pure ($ \"abc\") <*> pure show")

        assert ((pure (.) <*> pure length <*> pure show <*> pure 123) `eq` (pure length <*> (pure show <*> pure 123))) `withTitle` ("pure (.) <*> pure length <*> pure show <*> pure 123 == pure length <*> (pure show <*> pure 123)")
